...
export const metadata: Metadata = {
  title: "Arquivo — ZYPERIA OnlineBiz",
  description: "Todos os casos de estudo organizados por data de publicação.",
};

async function getArchivedArticles() {
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:3003"}/api/articles?limit=999&status=published`, {
      cache: "no-store",
    });
    if (!res.ok) return [];
  ...
  return (
    <main className="min-h-screen pt-32 pb-20">
      <div className="max-w-4xl mx-auto px-4">
        ...
        <h1 className="h-display text-4xl md:text-5xl mb-8">Todos os casos de estudo, por data.</h1>
              ...
              <div key={`${month.year}-${month.month}`}>
                ...
                <div className="flex items-baseline justify-between mb-6 pb-4 border-b border-white/10">
                  ...
                  <span className="text-xs h-mono uppercase tracking-wider text-[var(--text-muted)]">
                    {month.count} caso{month.count === 1 ? "" : "s"} de estudo
                  </span>
                </div>
