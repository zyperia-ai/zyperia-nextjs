...
export const metadata: Metadata = {
  title: "Casos de Estudo — ZYPERIA OnlineBiz",
  description: "Todos os casos de estudo sobre afiliados, produtos digitais, micro-SaaS, e rendimento online.",
};
...
async function getArticles(
  ...
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:3003"}/api/articles?${params}`, {
      cache: "no-store",
    });
  ...
  return (
    <main className="min-h-screen pt-32 pb-20">
      <div className="max-w-4xl mx-auto px-4">
        {/* Kicker + Title */}
        <span className="kicker mb-4">Casos de estudo</span>
        ...
        <p className="text-[var(--text-secondary)] mb-8">
          {total > 0
            ? `${total} caso${total === 1 ? "" : "s"} de estudo · página ${page} de ${totalPages}`
