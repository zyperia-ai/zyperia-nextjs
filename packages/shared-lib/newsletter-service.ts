...
/**
 * STEP 3: Send Newsletter via Resend (bulk, for monetization)
 * Supports large batch sends via Resend API
 *
...
export async function sendNewsletter(
  ...
  try {
    ...
    // Send via Resend (supports batch sends with Resend Team)
    const result = await getResend().emails.send({
      from: `ZYPERIA ${themeInfo.name} <newsletter@zyperia.ai>`,
      to: subscribers.map((sub: any) => sub.email),
    ...
    await getSupabase().from('generation_logs').insert({
      ...
      cost_usd: subscribers.length * 0.01, // Resend cost per email
